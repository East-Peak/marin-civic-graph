...
- first graph-materialization scaffold for the narrowed San Rafael governance spine:
  ...
  - graph-v1 is now at `6154` nodes and `20667` edges, with `1472` `Decision` nodes, `2970` `AgendaItem` nodes, `147` `MoneyFlow` nodes, `21` `EconomicInterestDisclosure` nodes, `8` `SeatService` nodes, `4` `Case` nodes, `13` `Proceeding` nodes, `9` `CaseParticipation` nodes, `1` `Program` node, and `16` bounded `ValidationCheck` nodes after projection merging
  ...
  - the current projection report still shows `missing_target:Actor = 149` and `missing_target:Record = 44`; the legal import did not widen the actor problem, and the reintroduced record misses are concentrated in older city-side campaign OCR/PDF artifacts from the accepted 2020 money batch
