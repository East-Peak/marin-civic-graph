...
export const EDGE_WEIGHTS: Record<string, number> = (() => {
  ...
  // DISCLOSED_AS/INTEREST_IN (COI spec §4.2, M4) join the governance tier —
  // COI paths walk Person→Filing→EconomicInterest→Organization.
  add(3, ["HELD_BY", "FOR_SEAT", "RESULT_OF", "CONTROLLED_BY", "BY_PERSON", "IN_ELECTION", "FOR_ELECTION", "FOR_PROJECT", "MEMBER", "MEMBER_OF_ORG", "DISCLOSED_AS", "INTEREST_IN"]);
  // Weight 4.
