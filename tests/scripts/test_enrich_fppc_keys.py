...
non-clobbering validating shim against the shared resolver view
(org_resolution.py stays byte-identical).
"""
...
def test_register_validates_committee_id_normalizer():
    register_committee_id_normalizer()
    assert enrich_fppc_keys.KEY_NORMALIZERS["committee_id"] is _normalize_committee_id
...
def test_register_refuses_a_foreign_registration(monkeypatch):
    monkeypatch.setattr(
        enrich_fppc_keys,
        "KEY_NORMALIZERS",
        MappingProxyType({"committee_id": lambda v: "foreign"}),
    )
