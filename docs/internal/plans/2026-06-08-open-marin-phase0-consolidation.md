...
> **Revised scope (2026-06-08) — C is CODE-ONLY; the live-graph overwrite folds into Phase 1.**
> Grounding found (a) there is **no master rebuild script** (`refresh_openmarin.py` is post-ingest only — a full overwrite means *building* an orchestrator over ~10 load scripts), and (b) **Phase 1 rebuilds the graph from source with the COI schema anyway**. So overwriting the live graph now (with pre-COI state) is throwaway + overlapping work. Since the live graph is disposable and Stuart doesn't need it current in the interim, C lands the consolidation as code and Phase 1 owns the one true rebuild.
> - **C1 — Port the v2 query pack:** `run_graph_query_pack.py` → importable `run_query_pack(projection_dir, schema="v2")` returning `{ok, failures, metrics}`, ported to settled labels + the repo's edge vocabulary. (Needed by Phase 1 / COI work too.)
> - **C2 — Retire the legacy projector:** delete `build_graph_projection.py` + `graph_projection_lib.py` (if v1-only) + `migrate_graph_v2.py` + `migration_mapping.py`; `build_graph_v2` (B-core, proven byte-equal) is the projector. `git grep` clean of live importers.
> - **C3 — Reconcile schema/docs:** `registry/neo4j-schema.cypher` matches `canonical_type.py`; README / query-pack docs / decision-log route to `build_graph_v2`, not the retired path.
> - **No DB writes in C.** The full fresh rebuild (overwrite live Aura with current state, via a master orchestrator over all sources) is a **Phase 1 task**, done once with the COI schema.
> - **`data/raw` backed up** locally (`~/backups/marin-civic-graph/data-raw-*.tar.gz`, sha256) — done, since the agenda/decision extractors only re-derive from local raw.
>
> **This callout SUPERSEDES the C1–C5 tasks below** (those still mention a `--local-only` refresh, deleting `migration_mapping`, and a final live equivalence — all stale). **Codex round 1 on the C prompt (2026-06-08)** further found: the retirement is a **full helper-closure extraction** (`build_graph_v2` reuses `build_actor_alias_map, build_node_envelope, extract_edges_from_object, finalize_edge/node_for_output, merge_edge/node, passthrough_relationships, remap_actor_aliases, should_include_object` + their deps `EXPLICIT_RELATION_TYPES`/`build_edge_envelope`/etc.), so a clean delete is non-trivial; the **golden parity fixture lives in gitignored `data/projected/`** and must be committed to `tests/fixtures/phase0/golden-current/` (else deleting the legacy path evaporates the regression gate); the **query pack is v1-hardcoded** (Q4 keys on `node_type=="Actor"`, needs `report.json`). **Recommendation: split C** — *C-land* (query-pack v2 port + commit goldens + delete dead `migrate_graph_v2` + schema/docs routing to `build_graph_v2`; mark `build_graph_projection` deprecated-but-reused) and a separate *legacy-extraction* follow-up (pure hygiene). Rework the C goal doc + this task list before running.
